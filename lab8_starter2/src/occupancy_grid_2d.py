...
class OccupancyGrid2d(object):
    ...
    def Sensor0Callback(self, msg):
        ...
        try:
            pose = self._tf_buffer.lookup_transform(
                self._fixed_frame, self._sensor0_frame, rospy.Time())
            print(pose)
        ...
        for idx, r in enumerate(msg.ranges):
            ...
            if r < msg.range_min:
                ...
                continue
            gridprev = []
            for i in np.arange(r, 0.0, -self._x_res/2.0):
            	x = i*np.cos(fixed_angle) + sensor_x
            	y = i*np.sin(fixed_angle) + sensor_y
            	voxel = self.PointToVoxel(x, y)
            	if voxel not in gridprev:
            		gridprev.append(voxel)
            		if i == r:
            			self._map[voxel[0]][voxel[1]] += self._occupied_update
            			if self._map[voxel[0]][voxel[1]] > self._occupied_threshold:
            				self._map[voxel[0]][voxel[1]] = self._occupied_threshold
            		else:
            			self._map[voxel[0]][voxel[1]] += self._free_update
            			if self._map[voxel[0]][voxel[1]] < self._free_threshold:
            				self._map[voxel[0]][voxel[1]] = self._free_threshold
    ...
    def Sensor1Callback(self, msg):
        ...
        try:
            pose = self._tf_buffer.lookup_transform(
                self._fixed_frame, self._sensor1_frame, rospy.Time())
            print(pose)
        except (tf2_ros.LookupException,
                tf2_ros.ConnectivityException,
                tf2_ros.ExtrapolationException):
    ...
    def Sensor2Callback(self, msg):
        ...
        try:
            pose = self._tf_buffer.lookup_transform(
                self._fixed_frame, self._sensor2_frame, rospy.Time())
            print(pose)
        except (tf2_ros.LookupException,
                tf2_ros.ConnectivityException,
                tf2_ros.ExtrapolationException):
    ...
    def Sensor3Callback(self, msg):
        ...
        try:
            pose = self._tf_buffer.lookup_transform(
                self._fixed_frame, self._sensor3_frame, rospy.Time())
        except (tf2_ros.LookupException,
                tf2_ros.ConnectivityException,
                tf2_ros.ExtrapolationException):
            # Writes an error message to the ROS log but does not raise an exception
    ...
    def Sensor4Callback(self, msg):
        ...
        try:
            pose = self._tf_buffer.lookup_transform(
                self._fixed_frame, self._sensor4_frame, rospy.Time())
        except (tf2_ros.LookupException,
                tf2_ros.ConnectivityException,
                tf2_ros.ExtrapolationException):
            # Writes an error message to the ROS log but does not raise an exception
    ...
    def Sensor5Callback(self, msg):
        ...
        try:
            pose = self._tf_buffer.lookup_transform(
                self._fixed_frame, self._sensor5_frame, rospy.Time())
        except (tf2_ros.LookupException,
                tf2_ros.ConnectivityException,
                tf2_ros.ExtrapolationException):
            # Writes an error message to the ROS log but does not raise an exception
    ...
    def Sensor6Callback(self, msg):
        ...
        try:
            pose = self._tf_buffer.lookup_transform(
                self._fixed_frame, self._sensor6_frame, rospy.Time())
        except (tf2_ros.LookupException,
                tf2_ros.ConnectivityException,
                tf2_ros.ExtrapolationException):
            # Writes an error message to the ROS log but does not raise an exception
    ...
    def Sensor7Callback(self, msg):
        ...
        try:
            pose = self._tf_buffer.lookup_transform(
                self._fixed_frame, self._sensor7_frame, rospy.Time())
        except (tf2_ros.LookupException,
                tf2_ros.ConnectivityException,
                tf2_ros.ExtrapolationException):
            # Writes an error message to the ROS log but does not raise an exception
